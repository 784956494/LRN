import torch
from manifold.utils import acosh, sqrt, clamp
...
def _logmap0(y, k, dim: int = -1):
    ...
    zero_point[0] = 1
    ret = (acosh(alpha) / torch.sqrt((alpha * alpha - 1).clamp_min(1e-8))) 
    a = (y - alpha * zero_point)
    assert(not alpha.isinf().any())
    assert(not ret.isinf().any())
    return ret * (y - alpha * zero_point)
